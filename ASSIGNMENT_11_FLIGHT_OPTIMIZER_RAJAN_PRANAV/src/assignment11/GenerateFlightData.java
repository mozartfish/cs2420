...
/**
 * This class will generate random flights for you to test. If you want to make your own just ignore
 * this file. You do not need to know anything about the implementation, you will not be tested on
 * it, it is just made to be a help if you are having a hard time generating files.
 *
 * <p>We suggest using this file to generate some random data then modify it slightly to test an
 * edge case. That way you have several other flights your algorithm will have to check as well as
 * the correct one.
 *
 * <p><b>USAGE:</b> You can use this class just by changing the commented global variables at the
 * top of the file. For students interested in what was done in lab with the command line you can
 * also compile it with <code>javac</code> and give it 3 command-line parameters: (1)The number of
 * edges you want in the graph, (2)the number of nodes in the graph, and (3)the name of the file for
 * the .csv file to output to. Keep in mind dense graphs will have shorter paths and sparse graphs
 * may not have paths to every node.
 *
...
public class GenerateFlightData {
  ...
  private static ArrayList<String> airliners =
      new ArrayList<String>(Arrays.asList("AA", "DL", "EV", "HA", "MQ", "OO", "UA", "WN"));
  private static Random r = new Random();

  private static HashMap<NodeDistance, Integer> flightDistances =
      new HashMap<NodeDistance, Integer>();
      ...
      for (int i = 0; i < edgeCount; i++) {
        ...
        String s =
            origin
                ...
                + "\n";
        if (i == edgeCount - 1) s = s.substring(0, s.length() - 1);
        writer.write(s);
      }
  ...
  private static String getRandomAirportDestination(String airport) {
    ...
    while (s.equals(airport)) s = airports.get(r.nextInt(airports.size()));
  ...
  private static String getRandomAirportOrigin() {
    ...
    while (addedAirports.contains(s)) s = airports.get(r.nextInt(airports.size()));
  ...
  public static void generateRandomAirports(int airportCount) {
    while (airports.size() < airportCount) {
      ...
      if (airports.contains(airport)) continue;
  ...
  private static int generateDistance(String origin, String destination) {
    NodeDistance n = new NodeDistance(origin, destination);
    if (flightDistances.containsKey(n)) return flightDistances.get(n);
